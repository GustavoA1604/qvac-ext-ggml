...
// Human-readable name for an OpenCL error code. The CL error space is a
// negative-integer enum the spec defines as macros in CL/cl.h, so we
// translate the common ones here and fall back to "CL_ERROR(<n>)" for
// anything not in the list. Used by `CL_CHECK` to bake the symbolic
// name into the `GGML_ABORT` message so the cause survives the
// tombstone path on platforms where the asynchronous `GGML_LOG_ERROR`
// channel is flushed lazily and would lose the message between the
// log call and `abort()` (notably Android, where addon-level log
// forwarding goes through an RPC queue that may not flush before the
// process exits).
static inline const char * ggml_cl_error_name(cl_int err) {
    switch (err) {
        ...
        default:                                           return "CL_ERROR(unknown)";
    }
}

// On a non-success CL return, log the error AND bake it into the
// abort message. The `GGML_LOG_ERROR` line is preserved for hosts
// that wire ggml's log channel into a synchronous sink (e.g.
// desktop stdout); on hosts whose log channel is asynchronous (e.g.
// the Bare-runtime addon on Android, which forwards GGML_LOG_*
// through an RPC queue that may not flush before `abort()`), the
// `GGML_ABORT` format string carries the same information into the
// process tombstone via `ggml_abort`'s synchronous `fprintf(stderr)`
// path, so the failure is still actionable from a crash dump.
#define CL_CHECK(err)                                                          \
    do {                                                                       \
        cl_int err_ = (err);                                                   \
        if (err_ != CL_SUCCESS) {                                              \
            GGML_LOG_ERROR("ggml_opencl: %s error %d (%s) at %s:%d\n",        \
                #err, err_, ggml_cl_error_name(err_), __FILE__, __LINE__);     \
            GGML_ABORT("ggml_opencl: %s returned CL error %d (%s) at %s:%d",   \
                #err, err_, ggml_cl_error_name(err_), __FILE__, __LINE__);     \
        }                                                                      \
    } while (0)
